...
                                                                            }'''
    ...
    def packagedCases(self, export_lable_title, data_key, class_name, pannel_num):
        ...
        cases_packaged = ''
        cases_packaged = ''.join(
            [cases_packaged, '                   <tr class="{}">\n'.format(class_name), export_lable_title,
             '                   </tr>\n'])
    ...
    def genHtmlReport(self, html_template):
        ...
        export_lable_datas = REPORT_TMPL_Lable.format(len(report_cases['all_cases']),
                                                      len(report_cases['right_cases']),
                                                      len(report_cases['error_cases']),
                                                      len(report_cases['untreaded_cases']),
                                                      str(str_all_cases[0:-1]), str(str_right_datas[0:-1]),
                                                      str(str_error_cases[0:-1]), str(str_untreated_cases[0:-1]))

        html_template = ''.join([html_template, export_lable_datas])

        html = template(html_template, report_title=self.html_title, theme=self.pie_theme, start_time=self.test_info[0],
                        end_time=self.test_info[1], used_time=self.test_info[
                2], sum_all_cases=
                        self.test_info[3], sum_executed_cases=self.test_info[4], sum_untreaded_cases=self.test_info[5],
                        right_sum=self.pie_sum_number['right_sum'], error_sum=self.pie_sum_number[
                'error_sum'], untreated_sum=
                        self.pie_sum_number['untreated_sum'])
        return html
...
if __name__ == '__main__':
    ...
    '''
    report_cases = {'all_cases': [['理财', '1', '登录',
                                                    'lo144444444444444444444444444444444444444444444444444444444444444gin',
                                                    'post', 'Pass', '详细', case_detail],['WEB理财', '2', '产品列表', 'list', 'post', 'Fail', '详细', case_detail],['理财', '3', '测试', 'login',
                                                    'po6666666666666666666666666666666666666666666666666666st', '/',
                                                    '详细', case_detail]],'right_cases': [
        ['理财', '1', '登录', 'login', 'po22222222222222222222222222222st', 'Pass', '详细', case_detail]],'error_cases': [
        ['理财', '2', '产品列表', 'list', 'pos5555555555555555555555t', 'Fail', '详细', case_detail]],'untreaded_cases': [
        ['WEB理财', '3', '测试', 'login', 'po6666666666666666666666666666666666666666666666666666st', '/', '详细',
         case_detail]]}

    sss=json.dumps(report_cases)
    ssss=json.dumps(case_detail)
    print(sss)
    print(case_detail)
